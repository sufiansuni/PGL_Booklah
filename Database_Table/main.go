...
func GetRecords(db *sql.DB) {
	...
	results, err := db.Query("Select * FROM my_table.Table")
	if err != nil {
		panic(err.Error())
	}
	for results.Next() {
...
func InsertRecord(db *sql.DB, Name string, TableID string, Booking string, Capacity int) {
	results, err := db.Exec("INSERT INTO my_table.Table VALUES (?,?,?,?)",
...
func EditRecord(db *sql.DB, Name string, TableID string, Booking string, Capacity int) {
	results, err := db.Exec(
		"UPDATE Course SET TableID=?, Booking=? , Capacity=?  WHERE Name=?",
...
func main() {
	db, err := sql.Open("mysql", "user:password@tcp(127.0.0.1:64810)/my_table")
	defer db.Close()
